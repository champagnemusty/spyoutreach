...
import { ArrowRight, Check, FileText, Radar, Sparkles, Upload, Zap } from "lucide-react";
...
const FEATURES = [
  {
    ...
    title: "Lead Cleanser",
    description:
      "Upload a raw B2B list (CSV or JSON). We strip legal suffixes from company names and flag malformed emails — clean data out, in seconds.",
  },
  {
    icon: FileText,
    title: "Spy Brief Generator",
    description:
      "Drop in a competitor's name or URL and get a 5-page, print-ready ad strategy brief — positioning, creative breakdown, messaging angles, and a recommended play.",
  },
];
  ...
  return (
    <main className="flex min-h-screen flex-col">
      ...
      <section className="mx-auto w-full max-w-3xl px-6 py-24 text-center">
        ...
        <p className="mt-4 text-xs text-muted">3 free credits on sign up. No credit card needed.</p>
      </section>

      <section className="mx-auto w-full max-w-5xl px-6 pb-24">
        <div className="grid gap-5 sm:grid-cols-2">
          {FEATURES.map((feature) => (
            <LuxuryCard key={feature.title} className="p-6">
      ...
      <section id="pricing" className="mx-auto w-full max-w-5xl scroll-mt-20 px-6 pb-24">
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          {PACKAGES.map((pkg) => (
            <LuxuryCard key={pkg.name} className="flex flex-col p-6">
              {pkg.highlight && (
